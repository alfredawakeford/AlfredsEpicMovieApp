...
async function navigateEpisode(direction) {
  ...
  const defaultSrc = `https://www.vidking.net/embed/tv/${id}/${s}/${e}?color=e50914&autoPlay=true`;
    ...
    if (item.media_type === "movie") {
      ...
      if (isInWatched && currentSeason && currentEpisode) {
        if (isCurrentUnreleased) {
          ...
          actionButtonsHTML = `
            <button class="play-btn" onclick="openVideoPlayer('https://www.vidking.net/embed/tv/${item.id}/${currentSeason}/${currentEpisode}?color=e50914&autoPlay=true', '${title} - S${currentSeason}E${currentEpisode}', ${item.id}, '${item.media_type}', '${title.replace(/'/g, "\\'")}', '${data.poster_path || ''}', ${currentSeason}, ${currentEpisode})">
        ...
        actionButtonsHTML = `
          <button class="play-btn" onclick="openVideoPlayer('https://www.vidking.net/embed/tv/${item.id}/1/1?color=e50914&autoPlay=true', '${title} - S1E1', ${item.id}, '${item.media_type}', '${title.replace(/'/g, "\\'")}', '${data.poster_path || ''}', 1, 1)">
                ...
                episodesContainer.innerHTML = seasonData.episodes.map(ep => {
                  ...
                  const playBtnHTML = isUnreleased 
                    ...
                    : `<button class="episode-play" title="Play episode"
                       onclick="openVideoPlayer('https://www.vidking.net/embed/tv/${item.id}/${seasonNum}/${ep.episode_number}?color=e50914&autoPlay=true', '${videoTitle}', ${item.id}, '${item.media_type}', '${title.replace(/'/g, "\\'")}', '${data.poster_path || ''}', ${seasonNum}, ${ep.episode_number})">
                       ▶
                       </button>`;
...
function updateModalUI(id, mediaType, title, nextSeason, nextEpisode) {
  ...
  if (modalActions) {
    modalActions.innerHTML = `
      <button class="play-btn" id="tempPlayBtn" onclick="openVideoPlayer('https://www.vidking.net/embed/tv/${id}/${nextSeason}/${nextEpisode}?color=e50914&autoPlay=true', '${title.replace(/'/g, "\\'")} - S${nextSeason}E${nextEpisode}', ${id}, '${mediaType}', '${title.replace(/'/g, "\\'")}', '${posterPath}', ${nextSeason}, ${nextEpisode})">
...
function updateModalToUnwatchedState(id, title) {
  ...
  if (modalActions) {
    modalActions.innerHTML = `
      <button class="play-btn" onclick="openVideoPlayer('https://www.vidking.net/embed/tv/${id}/1/1?color=e50914&autoPlay=true', '${title.replace(/'/g, "\\'")} - S1E1', ${id}, 'tv', '${title.replace(/'/g, "\\'")}', '${posterPath}', 1, 1)">
        ▶ Play Season 1 Episode 1
      </button>
      <div style="position: relative; display: inline-block;">
