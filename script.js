...
async function loadAlternateLinks() {
    try {
        ...
        lines.forEach((line, index) => {
            ...
            if (tmdbId && embedUrl) alternateLinks.set(tmdbId, embedUrl);
        });
    ...
    try {
        ...
        lines.forEach((line, index) => {
            ...
            if (parts.length >= 4) {
                ...
                tvAlternateLinks.set(`${id}_${season}_${episode}`, embedUrl);
            }
        });
...
function getAlternateLink(id, mediaType) {
    return mediaType === 'movie' ? alternateLinks.get(String(id)) || null : null;
}
function getTvAlternateLink(id, season, episode) {
    return tvAlternateLinks.get(`${id}_${season}_${episode}`) || null;
}
...
function loadVideoElement(url, id, mediaType, season, episode, startAtZero = false) {
    ...
    if (finalUrl.toLowerCase().endsWith('.mp4')) {
        ...
        video.addEventListener('ended', () => {
            savePlaybackPosition(id, mediaType, season, episode, 0);
...
async function openVideoPlayer(url, title, id, mediaType, itemTitle, posterPath, season = null, episode = null, startAtZero = false) {
    ...
    if (mediaType.trim() === "tv" && season && episode) {
        try {
            ...
            if (epData && epData.name) titleEl.textContent = `${itemTitle} - S${season}E${episode}: ${epData.name}`;
...
async function navigateEpisode(direction) {
    ...
    if (currentVideo && videoSaveInterval) {
        ...
        clearInterval(videoSaveInterval);
        ...
        if (item.media_type === "tv" && data.seasons?.length > 0) {
            ...
            for (const season of numberedSeasons) {
                ...
                modalHTML += `
                    ...
                    <div class="episodes-list" id="episodes-s${season.season_number}"><div class="episode-loading">Loading episodes...</div></div>
                `;
            }
            if (specialsSeason) {
                ...
                modalHTML += `
                    ...
                    <div class="episodes-list" id="episodes-s0"><div class="episode-loading">Loading episodes...</div></div>
                `;
            }
            modalHTML += `</div>`;
        }
        ...
        if (item.media_type === "tv") {
            document.querySelectorAll('.season-toggle').forEach(btn => {
                btn.onclick = async (e) => {
                    ...
                    const container = document.getElementById(`episodes-s${seasonNum}`);
                    ...
                    if (isActive && !container.dataset.loaded) {
                        try {
                            ...
                            if (seasonData.episodes?.length > 0) {
                                ...
                                container.innerHTML = '<div class="no-episodes">No episodes listed</div>';
                            }
                            container.dataset.loaded = "true";
                        } catch (err) {
                            console.error("Error loading season:", err);
                            container.innerHTML = '<div class="no-episodes" style="color:#e50914">Failed to load episodes</div>';
                        }
                    }
...
async function loadNewAdditions(append = false) {
    ...
    if (!append) { container.innerHTML = "<p>Loading...</p>"; newAdditionsPage = 1; }
...
document.getElementById("exportCsv")?.addEventListener("click", () => {
    ...
    if (items.length === 0) { alert("No watch history to export!"); return; }
    const headers = ["ID", "Title", "Type", "Season", "Episode", "Date Added", "Last Watched"];
    const rows = items.map(item => [
        item.id, `"${(item.title || item.name).replace(/"/g, '""')}"`, item.media_type,
        item.currentSeason || "N/A", item.currentEpisode || "N/A",
        new Date(item.addedAt).toLocaleString(), item.lastWatched ? new Date(item.lastWatched).toLocaleString() : "N/A"
    ].join(","));
    const csv = [headers.join(","), ...rows].join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url; a.download = `watch-history-${new Date().toISOString().split("T")[0]}.csv`;
...
document.getElementById("clearMoviesFromWatching")?.addEventListener("click", () => {
    if (confirm("Are you sure you want to remove all MOVIES from Continue Watching? TV shows will remain.")) {
        const watched = getWatchedData();
        for (const key in watched) { if (watched[key].media_type === "movie") delete watched[key]; }
        saveWatchedData(watched); displayContinueWatching();
    }
});

document.getElementById("clearData")?.addEventListener("click", () => {
    if (confirm("Are you sure? This will delete all watch history and watchlist!")) {
        localStorage.removeItem(STORAGE_WATCHED); localStorage.removeItem(STORAGE_WATCHLIST);
        displayContinueWatching(); displayWatchlist(); alert("All data cleared!");
    }
});
...
document.getElementById("clearContinueWatching")?.addEventListener("click", () => {
    if (confirm("Are you sure you want to clear all Continue Watching items?")) {
        localStorage.removeItem(STORAGE_WATCHED); displayContinueWatching(); alert("Continue Watching cleared!");
    }
});

document.getElementById("clearWatchlist")?.addEventListener("click", () => {
    if (confirm("Are you sure you want to clear your Watchlist?")) {
        localStorage.removeItem(STORAGE_WATCHLIST); displayWatchlist(); alert("Watchlist cleared!");
    }
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (movieModal) {
        window.onclick = (event) => { if (event.target === movieModal) movieModal.style.display = "none"; };
    }
